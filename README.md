...
> 2026-07-16：根路由现为 A 股单股决策驾驶舱，覆盖盘前、盘中、盘后三阶段。用户可从确定性候选池选择，也可按六位代码或已观察中文名称搜索；`?symbol=` 会在 A 股研究、新闻和回测间恢复同一标的，并作为风险、合规、审计页面的导航上下文。治理接口仍是全局或资产级数据，不提供单股过滤。可转债继续使用独立路由和状态。V1 仍在实施，浏览器验收和最终生产审计尚未完成。
...
- 任意已验证 A 股都可获得 `observe`、`wait` 或 `avoid` 即时研判，不要求先进入候选池。只有候选交易层及其行情、合规、证据、风险门禁全部通过后，才显示已有的模拟方案；非候选不会生成买入价、仓位或止损数值。
- 候选、搜索和本地自选共用一个 A 股标的；新闻默认过滤到当前股票，同时保留未绑定具体标的的市场级事件。
- 数据源缺失、陈旧、阻断或尚未运行时，分区显示原因并保留其他可用信息，不用空白或估算值冒充真实数据。
- 刑部、礼部和东厂展示全局或资产级治理状态；页面中的 A 股代码只表示导航来源，不代表这些记录已按单股过滤。
...
AI 解释层使用 OpenAI 兼容接口，只有同时配置以下三个环境变量时才会请求模型：
...
仓库不包含已配置的 AI 凭据或模型。未配置、超时、HTTP 错误、非法输出或证据引用越界时，页面会显示对应 AI 状态，并保留确定性即时研判。AI 只解释冻结证据，不能修改动作、置信度、价格、仓位或候选门禁。
...
本软件只用于观察、研究、回测和模拟验证，不构成投资建议。AI 只解释已冻结证据；价格、评分、观察动作和风险门禁由确定性程序产生。系统不连接真实券商、不自动交易、不执行真实买卖，也不承诺收益。
