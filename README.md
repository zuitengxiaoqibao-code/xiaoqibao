...
> 2026-07-15：根路由现为 A 股单股决策驾驶舱，覆盖盘前、盘中、盘后三阶段。用户可从确定性候选池选择，也可按六位代码或已观察中文名称搜索；`?symbol=` 会在 A 股研究、新闻和回测间恢复同一标的，并作为风险、合规、审计页面的导航上下文。治理接口仍是全局或资产级数据，不提供单股过滤。可转债继续使用独立路由和状态。V1 仍在实施，浏览器验收和最终生产审计尚未完成。
