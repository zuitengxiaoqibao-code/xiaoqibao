...
function PhaseContent({ slot, serverTime, staleAfter }: { slot: PhaseSlot; serverTime: string; staleAfter: number }) {
...
export function DecisionWorkbench({ loadCurrent, loadDate }: Props) {
  ...
  const [date, setDate] = useState(""); const [error, setError] = useState(""); const [loading, setLoading] = useState(true); const sequence = useRef(0); const manualPhase = useRef(false); const initialLoad = useRef(true);
  ...
  return <main className="decision-workbench"><header className="decision-header"><div><p className="eyebrow">A 股 · 每日决策工作台</p><h1>今日判断与三阶段跟踪</h1></div><label>交易日期<input aria-label="交易日期" type="date" value={date} onChange={(event) => { setDate(event.target.value); void load(event.target.value); }} /></label></header>
    ...
    {data && !error && <div id={`decision-panel-${phase}`} role="tabpanel" aria-labelledby={`decision-tab-${phase}`}><PhaseContent slot={data.phases[phase]} serverTime={data.server_time} staleAfter={data.polling.stale_after_seconds} /></div>}
  </main>;
}
