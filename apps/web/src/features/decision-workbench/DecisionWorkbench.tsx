...
function PhaseContent({ slot, serverTime, staleAfter, pollingStatus }: { slot: PhaseSlot; serverTime: string; staleAfter: number; pollingStatus?: string }) {
  ...
  return <div className="decision-columns"><section><div className={`quality-banner ${slot.phase_status}`}><b>{slot.phase_status === "blocked" ? "决策已拦截" : slot.phase_status === "partial" ? "数据部分可用" : "决策快照就绪"}</b><span>{slot.aggregate_version}</span></div>{stale && <div className="stale-banner">聚合数据已过期</div>}{slot.advice.length === 0 && <section className="decision-message compact"><h2>当前阶段没有可展示建议</h2><p>保留阶段质量状态，不补造建议。</p></section>}{slot.advice.map((item) => <AdvicePanel key={item.advice_id} advice={item} plans={slot.plans} readiness={slot.plan_readiness[item.advice_id]} />)}</section><aside className="decision-rail"><h3>阶段状态</h3><p><Bot size={14} />AI {slot.ai_status === "ready" ? "可用" : slot.ai_status === "unavailable" ? "不可用" : "未调用"}</p><p>质量 {slot.quality}</p><p>轮询 {pollingStatus ?? "uninitialized"}</p><p>当前观察 {slot.advice.length} · 本次变化 {slot.delta_advice?.length ?? 0}</p><p>变化版本 {slot.delta_version ?? "--"}</p><p>聚合版本 {slot.aggregate_version}</p>{slot.change_stream?.length ? <section className="change-stream" aria-label="盘中变化流"><h4>变化流</h4>{slot.change_stream.map((item) => <p key={item.snapshot_id}>#{item.sequence} · {item.delta_advice.length} 项 · {new Date(item.generated_at).toLocaleTimeString("zh-CN")}</p>)}</section> : null}</aside></div>;
}
...
export function DecisionWorkbench({ loadCurrent, loadDate }: Props) {
  ...
  const [date, setDate] = useState(""); const [error, setError] = useState(""); const [loading, setLoading] = useState(true); const sequence = useRef(0); const manualPhase = useRef(false); const initialLoad = useRef(true); const historicalMode = useRef(false);
  const load = useCallback(async (selectedDate?: string) => { const request = ++sequence.current; setLoading(true); setError(""); try { const next = selectedDate && loadDate ? await loadDate(selectedDate) : await loadCurrent(); if (request !== sequence.current) return; setData(next); if (initialLoad.current || !manualPhase.current) setPhase(next.current_phase); initialLoad.current = false; setDate(next.trading_date); } catch (caught) { if (request === sequence.current) setError(caught instanceof Error ? caught.message : "决策链路暂不可用"); } finally { if (request === sequence.current) setLoading(false); } }, [loadCurrent, loadDate]);
  useEffect(() => { void load(); }, [load]);
  useEffect(() => { if (!data || data.market_session !== "open" || !data.polling.focus_interval_seconds) return; const timer = window.setInterval(() => void load(historicalMode.current ? date : undefined), Math.max(1, data.polling.focus_interval_seconds) * 1000); return () => window.clearInterval(timer); }, [data?.market_session, data?.polling.focus_interval_seconds, date, load]);
  ...
  return <main className="decision-workbench"><header className="decision-header"><div><p className="eyebrow">A 股 · 每日决策工作台</p><h1>今日判断与三阶段跟踪</h1></div><div className="decision-date-controls"><label>交易日期<input aria-label="交易日期" type="date" value={date} onChange={(event) => { historicalMode.current = true; setDate(event.target.value); void load(event.target.value); }} /></label><button type="button" onClick={() => void load(historicalMode.current ? date : undefined)}><RefreshCw size={14} />刷新当前</button></div></header>
    ...
    {data && !error && <div id={`decision-panel-${phase}`} role="tabpanel" aria-labelledby={`decision-tab-${phase}`}><PhaseContent slot={data.phases[phase]} serverTime={data.server_time} staleAfter={data.polling.stale_after_seconds} pollingStatus={data.polling.status} /></div>}
  </main>;
}
