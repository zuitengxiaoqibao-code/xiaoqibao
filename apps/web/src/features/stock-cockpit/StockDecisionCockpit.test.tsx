...
describe("StockDecisionCockpit", () => {
  it("renders research advice, evidence, risks, and history without simulation UI", async () => {
    renderCockpit(() => Promise.resolve(snapshot()));
    expect(await screen.findByText("行情与趋势可用，保持观察。")).toBeInTheDocument();
    expect(screen.getAllByText("量价稳定").length).toBeGreaterThan(0);
    expect(screen.getAllByText("市场波动").length).toBeGreaterThan(0);
    expect(screen.queryByText(/模拟操作|止损参考|行情门禁/)).not.toBeInTheDocument();
  });
