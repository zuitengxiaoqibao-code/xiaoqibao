...
function WatchlistRow({ symbol, item, selected, onSelect, onWatch }: { symbol: string; item?: AShareInstrument; selected: boolean; onSelect: () => void; onWatch: () => void }) {
  return <article className={selected ? "stock-option selected" : "stock-option"}>
    <button type="button" aria-pressed={selected} onClick={onSelect}><span><b>{symbol}</b><small>{item?.name ?? "身份数据暂不可用"}</small></span><span><small>最新价不可用</small><small>当前涨跌不可用</small></span><span><small>{item ? `${item.exchange.toUpperCase()} · ${item.quote_quality}` : "可重试刷新身份"}</small>{item && <time>{new Date(item.observed_at).toLocaleString("zh-CN")}</time>}</span></button>
...
export function StockSelector({ loadCandidates, search, pollIntervalMs = 30_000 }: Props) {
  ...
  const refreshCandidates = useCallback(async (initial = false) => {
    ...
    try { setBoard(await loadCandidates()); setCandidateState("ready"); setCandidateError(""); }
    catch (reason) { setCandidateError(reason instanceof Error ? reason.message : "候选池加载失败"); setCandidateState("error"); }
  }, [loadCandidates]);

  useEffect(() => { void refreshCandidates(true); const timer = window.setInterval(() => void refreshCandidates(), pollIntervalMs); return () => window.clearInterval(timer); }, [pollIntervalMs, refreshCandidates]);
  useEffect(() => {
  ...
  const resolveSymbols = useCallback(async (symbols: string[]) => {
    ...
    const settled = await Promise.all(unique.map((symbol) => search(symbol).then((response) => response.items.find((item) => item.symbol === symbol)).catch(() => undefined)));
    settled.forEach((item, index) => { if (!item) identityRequests.current.delete(unique[index]); });
    setIdentities((current) => Object.fromEntries([...Object.entries(current), ...settled.filter((item): item is AShareInstrument => Boolean(item)).map((item) => [item.symbol, item])]));
  }, [search]);
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => executeSearch(normalized), /^\d{6}$/.test(normalized) ? 0 : 250);
  ...
  function onTabKey(key: string) { if (!key.startsWith("Arrow")) return; const order: CandidateTab[] = ["short_term", "swing", "watchlist"]; const offset = key === "ArrowLeft" || key === "ArrowUp" ? -1 : 1; selectTab(order[(order.indexOf(tab) + offset + order.length) % order.length]); }
  const tabs = [["short_term", "短线候选"], ["swing", "波段候选"], ["watchlist", "自选股"]] as const;

  return <section className="stock-selector" aria-label="A 股选择器">
    <header><div><p className="eyebrow">A SHARE / SELECTOR</p><h2>选择观察股票</h2></div><span>{selection.symbol ?? "尚未选择 A 股"}</span><button type="button" className="selector-refresh" aria-label="刷新候选" onClick={() => void refreshCandidates()}><RefreshCw size={14} /></button></header>
    ...
    <div className="selector-tabs" role="tablist" aria-label="A 股候选分类">{tabs.map(([key, name]) => <button ref={(node) => { tabRefs.current[key] = node; }} id={`stock-selector-tab-${key === "short_term" ? "short" : key}`} key={key} role="tab" aria-controls="stock-selector-panel" aria-selected={tab === key} tabIndex={tab === key ? 0 : -1} type="button" onKeyDown={(event) => onTabKey(event.key)} onClick={() => selectTab(key)}>{name}</button>)}</div>
    <div id="stock-selector-panel" role="tabpanel" aria-labelledby={`stock-selector-tab-${tab === "short_term" ? "short" : tab}`} className="stock-options">
      ...
      {tab === "watchlist" && watchlist.map((symbol) => <WatchlistRow key={symbol} symbol={symbol} item={identities[symbol]} selected={selection.symbol === symbol} onSelect={() => selection.select(symbol, "watchlist")} onWatch={() => toggleWatch(symbol)} />)}
    </div>
  </section>;
}
