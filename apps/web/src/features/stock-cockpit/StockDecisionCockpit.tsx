...
const quoteNames = { ready: "行情有效", stale: "行情陈旧", unavailable: "行情不可用" } as const;
...
function AdviceConclusion({ advice, qualityReason }: { advice: Advice | null; qualityReason: string }) {
  ...
  return <section className="cockpit-conclusion"><header><div><p className="eyebrow">AI 解释 / 确定性规则</p><h2>当前判断</h2></div><div className="advice-confidence"><span>确定性置信度</span><strong>{Math.round(Number(advice.confidence) * 100)}%</strong></div></header>
    <div className="advice-verdict"><div><span>{advice.horizon === "intraday" ? "盘中优先" : "波段观察"}</span><h3>{advice.conclusion}</h3><p>{advice.plain_language_explanation || "AI 解释暂不可用，保留确定性结论。"}</p></div><div className="observe-seal"><ShieldAlert size={18} /><b>仅观察</b><small>未形成权威门禁方案</small></div></div>
    <div className="advice-evidence-grid">
      ...
      <section className="contrary"><h3><AlertTriangle size={15} />反方证据</h3>{advice.contrary_evidence.length ? advice.contrary_evidence.map((item) => <p key={item.evidence_id}>{item.summary}</p>) : <p>暂无已验证反方证据</p>}</section>
...
export function StockDecisionCockpit({ load }: Props) {
  ...
  return <main className="stock-cockpit">
    <header className="cockpit-identity"><div><p className="eyebrow">A 股单股决策驾驶舱 / {data.instrument.exchange.toUpperCase()}</p><h1>{data.instrument.name} <span>{data.symbol}</span></h1><div className="cockpit-quote"><strong>{displayNumber(metric(data, "latest_price"))}</strong><span className={positive ? "positive" : "negative"}>{Number.isFinite(change) ? positive ? <TrendingUp size={15} /> : <TrendingDown size={15} /> : null}{displayNumber(metric(data, "change_percent"), "%")}</span></div></div>
      <div className="cockpit-quality"><div><Clock3 size={15} /><span>行情时间</span><b>{new Date(data.instrument.observed_at).toLocaleString("zh-CN", { hour12: false })}</b></div><div><span>快照质量</span><b>{qualityNames[data.overall_quality]}</b><small>{quoteNames[data.instrument.quote_quality]}</small></div><button type="button" aria-label="刷新驾驶舱" onClick={() => void refresh()} disabled={loading}><RefreshCw size={15} className={loading ? "spin" : ""} /></button></div>
    </header>
