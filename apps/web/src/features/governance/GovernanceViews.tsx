...
type Props = { view: "xingbu" | "libu" | "dongchang"; loadRisk?: () => Promise<RiskStatus>; loadCompliance?: () => Promise<ComplianceStatus>; loadAudit?: () => Promise<AuditStatus>; complianceAction?: (source: string, action: "authorize" | "revoke" | "acknowledge") => Promise<unknown> };
export function GovernanceView(props: Props) {
  const [data, setData] = useState<RiskStatus | ComplianceStatus | AuditStatus | null>(null); const [error, setError] = useState("");
  const [acting, setActing] = useState<string | null>(null);
  async function reload() {
    const loader = props.view === "xingbu" ? props.loadRisk : props.view === "libu" ? props.loadCompliance : props.loadAudit;
    if (!loader) { setError("服务未配置"); return; }
    setError("");
    try { setData(await loader()); }
    catch (reason) { setError(reason instanceof Error ? reason.message : "服务不可用"); }
  }
  useEffect(() => { setData(null); void reload(); }, [props.view, props.loadRisk, props.loadCompliance, props.loadAudit]);
  ...
  async function runComplianceAction(source: string, action: "authorize" | "revoke" | "acknowledge") {
    ...
    try { await props.complianceAction(source, action); setData(await props.loadCompliance()); }
    catch (reason) { setError(reason instanceof Error ? reason.message : "合规操作失败"); }
