...
export function GovernanceView(props: Props) {
  ...
  async function reload() {
    ...
    setError("");
    try { setData(await (props.view === "libu" && props.loadCompliance ? props.loadCompliance(asset) : loader())); }
    catch (reason) { setError(reason instanceof Error ? reason.message : "服务不可用"); }
  }
  useEffect(() => { setData(null); void reload(); }, [props.view, props.loadRisk, props.loadCompliance, props.loadAudit, asset]);
  ...
  async function runComplianceAction(source: string, action: "authorize" | "revoke" | "acknowledge") {
    ...
    try { await props.complianceAction(source, action, references[source], asset); setData(await props.loadCompliance(asset)); }
