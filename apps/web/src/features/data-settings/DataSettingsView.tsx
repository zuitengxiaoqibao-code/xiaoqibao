...
export function DataSettingsView({ loadAI, saveAI, deleteAI, symbol, prepare, preparation: initialPreparation, onAIChanged }: Props) {
  ...
  useEffect(() => () => { mounted.current = false; aiGeneration.current += 1; prepGeneration.current += 1; aiController.current?.abort(); prepController.current?.abort(); }, []);
  ...
  useEffect(() => { prepGeneration.current += 1; prepController.current?.abort(); setPreparation(initialPreparation?.symbol === symbol ? (initialPreparation ?? null) : null); setMessage(""); }, [initialPreparation, symbol]);

  const retry = async () => { if (!symbol || !prepare) return; const request = ++prepGeneration.current; prepController.current?.abort(); const controller = new AbortController(); prepController.current = controller; setBusy(true); setMessage(""); try { const value = await prepare(symbol, controller.signal); if (mounted.current && request === prepGeneration.current && !controller.signal.aborted && value.symbol === symbol) setPreparation(value); } catch (error) { if (mounted.current && request === prepGeneration.current && !controller.signal.aborted) setMessage(friendlyError(error, "数据同步")); } finally { if (mounted.current && request === prepGeneration.current) setBusy(false); } };
  const save = async () => { const request = ++aiGeneration.current; aiController.current?.abort(); const controller = new AbortController(); aiController.current = controller; setBusy(true); setMessage(""); try { const value = await saveAI({ base_url: baseUrl, model, api_key: apiKey }, controller.signal); if (mounted.current && request === aiGeneration.current && !controller.signal.aborted) { applySettings(value); onAIChanged?.(); } } catch (error) { if (mounted.current && request === aiGeneration.current && !controller.signal.aborted) setMessage(friendlyError(error, "AI 设置")); } finally { if (mounted.current && request === aiGeneration.current) setBusy(false); } };
  const clear = async () => { const request = ++aiGeneration.current; aiController.current?.abort(); const controller = new AbortController(); aiController.current = controller; setBusy(true); setMessage(""); try { const value = await deleteAI(controller.signal); if (mounted.current && request === aiGeneration.current && !controller.signal.aborted) { applySettings(value); onAIChanged?.(); } } catch (error) { if (mounted.current && request === aiGeneration.current && !controller.signal.aborted) setMessage(friendlyError(error, "AI 设置")); } finally { if (mounted.current && request === aiGeneration.current) setBusy(false); } };
