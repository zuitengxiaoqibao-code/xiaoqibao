...
type Props = {
  ...
  loadCandidates: () => Promise<BondCandidates>;
};
...
export function ConvertibleBondView({ loadDashboard, loadDiagnosis, loadCandidates }: Props) {
  ...
  async function showCandidates() { setTab("candidates"); setLoading(true); setError(""); try { setCandidates(await loadCandidates()); } catch (e) { setError(e instanceof Error ? e.message : "候选池加载失败"); } finally { setLoading(false); } }
  ...
  return <main className="bond-domain">
    ...
    {error && <div className="bond-alert" role="alert"><ShieldAlert size={18} />{error}</div>}
    ...
    {tab === "diagnosis" && <section className="bond-diagnosis">
      ...
      {diagnosis && <>
        ...
        <section className="clause-evidence"><header><FileText size={17}/><div><b>{diagnosis.strong_redemption.state === "announced" ? "强赎已公告" : "强赎状态未知"}</b><small>{sourceName[diagnosis.strong_redemption.source] ?? diagnosis.strong_redemption.source} · {new Date(diagnosis.strong_redemption.observed_at).toLocaleString("zh-CN")}</small></div></header><p>{diagnosis.strong_redemption.clause_text}</p></section>
      </>}
    </section>}
    {tab === "candidates" && !loading && <section className="bond-candidates">{candidates?.items.length ? candidates.items.map((item) => <div key={item.bond_code}><b>{item.bond_code}</b><span>转股溢价 {value(item.conversion_premium)}</span><small>{item.risk.outcome}</small></div>) : <p>暂无符合条件的转债诊断快照</p>}</section>}
  </main>;
}
